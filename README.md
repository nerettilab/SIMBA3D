...
- To build cython replace the `setup.py` with `setup_cython.py` with the following command
```
cp setup_cython.py setup.py
```
- Uninstall any previous simba3d installations
```
pip3 uninstall simba3d
```
- You will need to have cython and numpy installed before you try to build the code.
```
pip3 install cython
pip3 install numpy
```
...
```
cp setup_pure_python.py setup.py
pip3 uninstall simba3d
pip3 install .
```
