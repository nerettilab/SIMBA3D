...
def h2apenalty(curve):
   ...
   # compute differences between ajacent nodes
    ...
    if transpose: # if the curve was transposed, to fit the function, transpose it again the match the original input
        gradh2=gradh2.T
    return h2,gradh2
def h2bpenalty(curve,pairwise_distance_matrix=None):
    '''
    ...
    h2b=0.0
    gradh2b=np.zeros(np.shape(curve))
    return h2b,gradh2b
def h2cpenalty(curve,pairwise_distance_matrix=None):
    '''
    Compute the h2b penalty and its gradient.
    ...
    # makes sure the curve is oriented correctly
    h2c=0.0
    gradh2c=np.zeros(np.shape(curve))
    return h2c,gradh2c
...
class opt_E():
    ...
    def initialize_gradient_manager(self):
        ...
        self.gradient_manager=gradient_manager(term_parameters)
    def set_contact_data(self,data):
        ...
        if 'population_contact_matrix' in data:
            ...
            '''
            Cpopdense=data['population_pairwise_contact_matrix']
        ...
        if (C is None)&(Cdense is not None) :
                ...
                '''
                pairwise_combinations=listIndices(self.n,self.off_diagonal)
        ...
        Ndata=sum(C.data)+10e-16
        Nprior=1
        if self.term_weights['population prior']:
            Nprior=sum(Cpop.data)+10e-16
            C+=self.term_weights['population prior']*(Ndata/Nprior)*Cpop
        self.pairwise_contact_matrix=C

        #keyboard()
        if self.term_weights['data']!=0:
            self.initialized_curve=scale_initialized_curve(self.initialized_curve,C,self.a,self.b)
        ...
        self.gradient_manager.set_contact_data(C.row.tolist(),C.col.tolist(),C.data.tolist())
    ...
    def energy_jacobian_native(self,curve,*args):
        ...
        if (self.term_weights['data']!=0)|(self.term_weights['population prior']!=0):
            e_data,gradient_data=analytical_data_gradient(curve.reshape(shp),self.pairwise_contact_matrix,self.a,self.b,offdiag=self.off_diagonal,idxmiss=self.index_missing,idxlist=self.pairwise_combinations)
