...
inputdir='data/'
datamatrix=['simulated_contact_matrix.npy']
...
#%%
UUID=uuid.uuid4()
for ii in range(len(datamatrix)):
    for jj in range(len(tab_indexes[ii])):
        missing_indexes=list(set(range(n[ii]))-set(tab_indexes[ii][jj]))
        tasks.append(
            {
                # task parameters        
                'usewarmstarts':True,
                ...
                'file_names'  :
                    ...
                    "pairwise_contact_matrix"   : datamatrix[ii],
                ...
                'index_parameters'  :
                    {
                        'index_mississing': missing_indexes
                    },
                # parameter settings
                'parameters'    :
                    {
